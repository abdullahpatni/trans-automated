...
test('Crossing agent bill excel download', async ({ page }) => {
    ...
    await page.getByRole('link', {
        name: 'Crossing Agent Bill Information', exact: true
    }).click();
    ...
    await page.locator('#mainReportType').selectOption('1');
    ...
    for (const regionOption of regionOptions) {
        ...
        if (!regionValue || regionValue === '0') continue;
        ...
        for (const subRegionOption of subRegionOptions) {
            ...
            if (!subRegionValue || subRegionValue !== '0') continue;
